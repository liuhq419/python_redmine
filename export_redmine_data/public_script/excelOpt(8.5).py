        ...
        try:
            ...
            if '所有人员' in excelName:
                ...
                pivoted_df_user = pd.pivot_table(sheet_df, values='耗时', index=['用户','所属部门'], columns=['活动', '难易度'],
                                                 aggfunc={'耗时': np.sum}, fill_value='', margins=True,
                                                 margins_name='总计')
                pivoted_df_user = pivoted_df_user.sort_values()  # 耗时排序
            ...
            bhdSheet = oldExcelFile.sheet_by_index(2)
            #获得复制后的表格工作簿
            ...
            if '所有人员' in excelName:
                name=sumSheet.col_values(0,6,bhdSheet.nrows-5)
                depart=sumSheet.col_values(1,6,bhdSheet.nrows-5)
                hour=sumSheet.col_values(bhdSheet.ncols,6,bhdSheet.nrows-4)
                workSheet=newExcelFile.add_sheet('工作量排序')
                workSheet.write(0,0,'姓名')
                workSheet.write(0,1,'所属部门')
                workSheet.write(0,2,'工作时间')
                for c in range(1,len(name)):
                    workSheet.write(0,c,name[c],style3)
                    workSheet.write(0,c,depart[c],style3)
                    workSheet.write(0,c,hour[c],style3)
