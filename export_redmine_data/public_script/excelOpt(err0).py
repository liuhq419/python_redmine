...
def readExcel(excelNames,PATH,newdeparts):
    ...
    for excelName in excelNames:
        ...
        try:
            ...
            sheet_pivoted_df.to_excel(writer, 'Summary')  # 将数据写入到sheet1中
            ...
            # 设置sheet1的总体格式
            for r2 in range(6, oldSheet1.nrows+sumSheet.nrows):
                for c2 in range(0, oldSheet1.ncols):
                    cell = oldSheet1.cell_value(r2, c2)
                    newSheet1.write(r2, c2, cell, style3)

            # 将明细数据写入到汇总的数据表中sheet1
            for i in range(0, sumSheet.nrows):
                for j in range(0, sumSheet.ncols):
                    cell = sumSheet.cell_value(i, j)
                    newSheet1.write(newSheet1.nrows + i + 1, j, cell, style3)
            ...
            newSheet1.remove_merged_ranges(oldSheet1.nrows+2, oldSheet1+2, oldSheet1.ncols - 2, oldSheet1.ncols - 1)
            newSheet1.remove_merged_ranges(oldSheet1.nrows+sumSheet.nrows-2, oldSheet1.nrows+sumSheet.nrows-1,1,1)
            ...
            newSheet1.write_merge(oldSheet1.nrows+1, oldSheet1.nrows+2, oldSheet1.ncols - 1, oldSheet1.ncols - 1, '总计',style2)
            ...
            newSheet1.write(0, 0, depart, style1)
            ...
            newSheet1.write_merge(2, 4, 0, 1, '人员/汇总', style2)
            newSheet1.write_merge(sumSheet.nrows+1,sumSheet.nrows+ 2,0,1,'人员/项目')
            ...
            for fm in range(0, timeSheet.nrows):
                newTimeSh.row(fm).set_style(tall_style)  # timelog的行高
            newTimeSh.col(0).width = 256 * 35  # 列宽
            newTimeSh.col(1).width = 256 * 16  # 列宽
            newTimeSh.col(4).width = 256 * 30  # 列宽
            for fm in range(0,oldSheet1.nrows+sumSheet.nrows):
                newSheet1.row(fm).set_style(tall_style)    #sheet1的行高
