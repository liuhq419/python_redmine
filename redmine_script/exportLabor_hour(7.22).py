...
def getHourSpend():
    ...
    def operateLaborHour(excelNames,pathExcelNames):
        ...
        for pathExcelName in pathExcelNames:
            ...
            newExcel = copy(excel)
            proSheet=newExcel.add_sheet('sheet1',cell_overwrite_ok=True)  #添加项目耗时工作簿
            proSheetTemp=newExcel.add_sheet('sheet2',cell_overwrite_ok=True)  #添加项目耗时工作簿
            ...
            setProSheetStyle(activityTypes, proSheet, name, date)
            #读取人员耗时表中的数据
            user=''
            ...
            count=4
            cellValue=0
            #对导出的项目耗时表进行操作，删除重复项目，计算总耗时
            file=xlrd.open_workbook(pathExcelName,formatting_info=True)
            newFile=copy(file)
            proSheetTemp=file.sheet_by_index(3)
            proSheet=newFile.get_sheet(2)
            try:
                project=proSheetTemp.row_values(5)[0]  #有错误
            except:return
            for row in range(5,proSheetTemp.nrows):
                rowValue=proSheetTemp.row_values(row)
                if project==rowValue[0]:
                    count+=1
                else:
                    project = rowValue[0]
                # cell=sum(proSheetTemp.col_values(col,row,row+count))

                '''
                if project==rowValue[0]:
                    count+=1
                    #7.21 需要修改#
                    for cell in range(1,len(rowValue)):
                        if rowValue[cell]:
                            cellValue+=float(rowValue[cell])
                        proSheet.write(row,0,project)
                        proSheet.write(row,count,cellValue,style3)
                else:
                    count+=1
                    for cell in range(0, len(rowValue)):
                         proSheet.write(row,count,rowValue[cell])
                    project=rowValue[0]'''
