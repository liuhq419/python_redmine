...
def sortWorkHours():
    ...
    topTen=str(sorts_df.iloc[9, 1])  # 选取第十行，第二列的值，返回的为单个值
    topTenBellow=sorts_df.tail(n=end)
    tailTenBellow=sorts_df.head(n=end)
    tailTen=str(sorts_df.iloc[end, 1]) # 选取第十行，第二列的值，返回的为单个值
    print(end,topTen,tailTen)
    topTenC=len(topTenBellow.query('有效工时=='+topTen+''))
    tailTenC=len(tailTenBellow.query('有效工时=='+tailTen+''))
    print(topTenC,tailTenC)
    topn=10+topTenC
    tailn=10+tailTenC
    ...
    for r2 in range(1, sortSheet.nrows):
        ...
        if 1 < r2 <= topn + 1:
            style = styBH1
        elif r2 >= sortSheet.nrows - tailn:
            style = styBH3
        else:
            style = style3
        for c2 in range(0, sortSheet.ncols):
            newSort.col(c2).width = 256 * 18  # 列宽
            newSort.write(r2, c2, sortSheet.cell_value(r2, c2), style)
    ...
    df1=sorts_df.head(n=topn)  # 查看前几行的数据,默认前5行
    df2=sorts_df.tail(n=tailn)  # 查看后几行的数据,默认后5行
    df=df1.append(df2)
    work_df=sorts_df.有效工时
    mean_df=work_df.mean()
    y=[]
    ...
    trace1 = go.Scatter(x=df.姓名, y=y, mode='lines', name='平均工作量',
                        line=dict(color='rgba(150, 0, 0, 1.0)', width=1, ),)
    layout_comp = go.Layout(
